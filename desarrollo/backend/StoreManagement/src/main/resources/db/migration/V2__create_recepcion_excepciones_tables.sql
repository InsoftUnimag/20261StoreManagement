-- V3: Crear tablas para Recepción de Mercancía y Excepciones de Inventario
...
CREATE TABLE detalle_manifiesto (
    ...
    CONSTRAINT fk_detalle_sku FOREIGN KEY (sku_id)
...
CREATE TABLE lote (
    ...
    CONSTRAINT fk_lote_sku FOREIGN KEY (sku_id)
        REFERENCES producto (sku_id),
...
CREATE TABLE stock_global_sku (
    ...
    CONSTRAINT fk_stock_global_sku FOREIGN KEY (sku_id)
        REFERENCES producto (sku_id)
);
...
CREATE TABLE excepcion_inventario (
    ...
    CONSTRAINT fk_excepcion_sku FOREIGN KEY (sku_id)
        REFERENCES producto (sku_id)
);
